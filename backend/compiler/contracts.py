...
class CapabilityContractAcquirer:
    """Acquire only explicitly referenced public API contracts; never invent endpoints."""
    ...
    def acquire(
        ...
        for source in context.sources:
            if source.kind == "api_spec":
                ...
                continue
            if source.kind == "url" and source.uri:
        ...
        for evidence in context.research_evidence:
            ...
            for ref in evidence.get("source_refs", []):
                if not isinstance(ref, str) or not ref.startswith("https://"):
                    continue
                if any(existing[2] == ref for existing in candidates):
                    continue
                candidates.append((f"research-url:{ref}", "", ref))
        ...
        for source_id, text, uri in candidates[: self.max_sources]:
            ...
            except (IngestionError, OpenAPICompileError, OSError, ValueError) as exc:
    ...
    def _fetch(url: str) -> IngestedSource:
        return asyncio.run(ingest_url(url, name=f"API contract: {url}"))
