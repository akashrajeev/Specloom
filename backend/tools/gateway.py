...
class ToolGateway:
    ...
    def invoke(self, invocation: ToolInvocation, *, approved: bool = False) -> dict[str, Any]:
        if invocation.capability:
            side_effecting = bool(invocation.capability.get("side_effecting"))
            if side_effecting and not approved:
                raise PermissionError(f"tool {invocation.tool_id} requires explicit approval")
            if invocation.mode in {"mock", "sandbox"}:
                return {"status": "simulated", "tool": invocation.tool_id, "input": invocation.input}
            if str(invocation.capability.get("kind")) == "openapi":
                return invoke_openapi_capability(invocation.capability, invocation.input, approved=approved)
        ...
        if invocation.mode in {"mock", "sandbox"}:
            return {"status": "simulated", "tool": invocation.tool_id, "input": invocation.input}
        return self.registry.invoke(invocation.tool_id, invocation.input, allow_side_effects=approved)
